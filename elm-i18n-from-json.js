...
function generate(filepath, options) {
    ...
    const tokens = _.toPairs(flatJson).map(([key, value]) => {
    ...
    const entries = _.toPairs(flatJson).map(([key, value]) => {
        return dotToCaps(escape(key))
    })

    const defaults = _.toPairs(flatJson).map(([key, value]) => {
        return `${dotToCaps(escape(key))} = "${key}"`
    })

    const lookups = _.toPairs(flatJson).map(([key, value]) => {
        return `${dotToCaps(escape(key))} = ${toToken(key)}`
    })

    const decodes = _.toPairs(flatJson).map(([key, value]) => {
    ...
    const strings = _.toPairs(flatJson).map(([key, value]) => {
...
function validateAction(configPath) {
    ...
    const pairs = _.toPairs(configJson.languages).map(([language, filename]) => {
...
function main(args) {
    ...
    program.command('validate <config>').action(validateAction)
